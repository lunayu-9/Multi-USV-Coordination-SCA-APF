function experiment_100_runs_traditional_apf()
    %% 实验参数设置
    ...
    sensing_range = 15; % 感知范围（同时作为斥力影响范围 d0）
    d0 = sensing_range;
    len_step = 0.3;     % 每步移动距离
    ...
    for exp = 1:total_experiments
        ...
        while norm(current_pos - goal) >= 0.3 && iter < max_iter
            ...
            % 合成力（传统APF：吸引力与斥力直接叠加）
            F_sum = F_att + F_rep;
            F_norm = norm(F_sum);
            if F_norm ~= 0
                UnitVec_Fsum = F_sum / F_norm;
            else
                UnitVec_Fsum = [cos(prev_heading), sin(prev_heading)];
            end
            
            % 根据合成力方向直接更新航向与位置
            new_heading = atan2(UnitVec_Fsum(2), UnitVec_Fsum(1));
            % 计算本步转向角（用于统计记录）
            delta_heading = new_heading - prev_heading;
            delta_heading = mod(delta_heading + pi, 2*pi) - pi;  % 归一化到 [-pi, pi]
